...
import com.ericlam.mc.bungee.hnmc.config.VersionCheckerConfig;
...
public class VersionUpdateListener implements Listener {
    ...
    public VersionUpdateListener(HyperNiteMC hyperNiteMC) {
        ProxyServer.getInstance().getScheduler().schedule(hyperNiteMC, new CheckerRunnable(hyperNiteMC), HyperNiteMC.getHnBungeeConfig().getVersionChecker().intervalHours, TimeUnit.HOURS);
    }
    ...
    public void onPlayerJoin(PostLoginEvent e) {
        ...
        updateMap.forEach((plugin, versions)->{
            ...
            e.getPlayer().sendMessage(HyperNiteMC.getHnBungeeConfig().getPrefix() + "§c 插件更新: " + plugin + " v" + version[1] + ", 最新版本: v" + version[0]);
        });
    }
    ...
    private class CheckerRunnable implements Runnable {
        ...
        public CheckerRunnable(HyperNiteMC hyperNiteMC) {
            this.config = HyperNiteMC.getHnBungeeConfig().getVersionChecker();
    ...
    public static boolean versionNewer(String versionCurrent, String versionLatest) {
        var unequal = HyperNiteMC.getHnBungeeConfig().getVersionChecker().use_unequal_check;
