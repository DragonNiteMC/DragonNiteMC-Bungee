package com.ericlam.mc.bungee.hnmc.implement;
...
public class ChatRunnerClicker implements ChatRunnerManager, Listener {
    ...
    public void registerTimeout(UUID id, ChatRunner runner, int timeout) {
        runnerMap.put(id,runner);
        ProxyServer.getInstance().getScheduler().schedule(HyperNiteMC.plugin,()->runnerMap.remove(id),timeout, TimeUnit.SECONDS);
    }
    ...
    public void onCommandPreProcess(ChatEvent e) {
        ...
        if (runner == null) {
            MessageBuilder.sendMessage(player,HyperNiteMC.getAPI().getMainConfig().getPrefix()+"&c文字點擊已過期。");
            return;
        }
        runner.run(player);
