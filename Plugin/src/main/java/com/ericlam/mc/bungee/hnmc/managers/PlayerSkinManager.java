...
import com.ericlam.mc.bungee.hnmc.HttpRequest;
...
public class PlayerSkinManager implements SkinValueManager {
    ...
    private CompletableFuture<PlayerSkin> updateSkinTask(Connection connection, UUID uuid){
        return HttpRequest.getSkinProperty(uuid).thenCombineAsync(HyperNiteMC.getAPI().getPlayerManager().getOfflinePlayer(uuid),(skinProperty, offlinePlayer) -> {
